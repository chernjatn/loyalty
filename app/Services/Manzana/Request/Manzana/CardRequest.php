    ...
    public function processRequest(): ?Collection
    {
        return transform(
            ...
            function (array $cards) {
                $cardsFiltered = collect($cards)->filter(fn (array $cardInfo) => $cardInfo['StatusCode'] == self::CARD_AVAILABLE_STATUS)
                    ->map(
                        fn (array $cardInfo) => new LoyCard(
                            $cardInfo['Number'],
                            $this->getLoyCardType($cardInfo),
                            empty($cardInfo['ActiveBalance']) || $cardInfo['ActiveBalance'] <= 1 ? null : moneyFromDecimal($cardInfo['ActiveBalance'])
                        )
                    );
